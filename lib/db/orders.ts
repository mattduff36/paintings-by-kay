...
async function ensureOrdersTable(): Promise<void> {
  ...
  await pgQuery(`alter table orders add column if not exists tracking_number text;`).catch(() => ({} as any));
}
...
export async function updateOrderStatus(id: string, status: Order['status'], trackingNumber?: string | null): Promise<Order> {
  await ensureOrdersTable();
  const setDespatched = status === 'despatched' ? ', despatched_at = now()' : '';
  const setTracking = typeof trackingNumber === 'string' ? ', tracking_number = $3' : '';
  const params: any[] = [status, id];
  if (typeof trackingNumber === 'string') params.push(trackingNumber);
  const sql = `update orders set status = $1, updated_at = now()${setDespatched}${setTracking} where id = $2 returning *`;
  const { rows } = await pgQuery<Order>(sql, params);
  return rows[0];
}

export async function deleteOrdersBySessionIds(sessionIds: string[]): Promise<number> {
  await ensureOrdersTable();
  if (!sessionIds.length) return 0;
  const placeholders = sessionIds.map((_, i) => `$${i + 1}`).join(',');
  const { rows } = await pgQuery<{ count: string }>(
    `delete from orders where stripe_session_id in (${placeholders}) returning 1 as count`,
    sessionIds,
  );
  return rows.length;
}
